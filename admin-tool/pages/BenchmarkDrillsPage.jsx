// pages/BenchmarkDrillsPage.jsx
// Page for managing benchmark drills
...
import {
  getAllBenchmarkDrills,
  deleteBenchmarkDrill,
  duplicateBenchmarkDrill,
} from '../lib/drillService';

export default function BenchmarkDrillsPage({ onNavigateBack }) {
  ...
  const loadDrills = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getAllBenchmarkDrills();
      setDrills(data);
    } catch (err) {
  ...
  const handleDuplicate = async (drill) => {
    try {
      await duplicateBenchmarkDrill(drill.id);
      loadDrills();
    } catch (err) {
  ...
  const handleDeleteClick = async (drill) => {
    ...
    setError(null);
    try {
      await deleteBenchmarkDrill(drill.id);
      loadDrills();
    } catch (err) {
